...
def load_data_v3(csv_path):
    ...
    for i_frame, frame in enumerate(data):
        ...
        head_trans_transformed[i_frame, 0] = 0.88652*head_transs[i_frame, 0] + 3.99025*head_transs[i_frame, 0] -0.19968*head_transs[i_frame, 0]
        head_trans_transformed[i_frame, 1] = 0.0332397*head_transs[i_frame, 0] + 1.0971145*head_transs[i_frame, 0] -0.0095729*head_transs[i_frame, 0]
        head_trans_transformed[i_frame, 2] = 0.34035*head_transs[i_frame, 0] + 1.18155*head_transs[i_frame, 0] +0.99748*head_transs[i_frame, 0]
...
def load_pv_info(csv_path):
    ...
    for i_frame, frame in enumerate(lines[3928:3929]):
        # Row format is
...
def plot_points(folder,frame_timestamps, pv_intrinsics, pv2world_rotation, pv_extrinsics, timestamps, gaze_data, gaze_available, head_transs, left_index_tip, left_middle_tip, left_index_tip_available, right_index_tip, right_middle_tip, right_index_tip_available):
    ...
    for pv_id in range(n_frames):
        ...
        try:
            pvinfo_frame = int(str(pv_path.name).replace('.jpg', ''))
        except:
            continue
